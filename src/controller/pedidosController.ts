...
const evaluarPedidosPendientes = async (idStock: string): Promise<void> => {
  try {
    ...
    const pedidosPendientes = stock.pedidos.filter(
      ...
      if (stock && modelo && modelo.placas_por_metro) {
        ...
        const stockReservado = stock.pedidos
          ? stock.pedidos
            .filter(
              (pedido: any) =>
                pedido.estado === "reservado" || pedido.estado === "pendiente"
            )
            .reduce(
              (total: number, pedido: any) => total + (pedido.cantidad || 0),
              0
            )
          : 0;
...
export const updatePedido = async (
  ...
  try {
    ...
    if (updates.productos && Array.isArray(updates.productos)) {
      ...
      console.log(
        "🗺️ Mapa de productos anteriores:",
        Object.fromEntries(productosAnterioresMap)
      );

      // ✅ 1. Detectar productos eliminados
      const productosEliminados = productosAnteriores.filter(
        (prodAnterior: any) =>
          !productosNuevos.some(
            (prodNuevo: any) =>
      ...
      if (productosEliminados.length > 0) {
        ...
        for (const productoEliminado of productosEliminados) {
          ...
          if (!modelo || !modelo.placas_por_metro) {
            console.warn(
              `⚠ No se encontró modelo para idModelo: ${productoEliminado.idModelo}`
            );
            continue;
          }

          const cantidadRealEliminada =
            productoEliminado.cantidad * modelo.placas_por_metro;
          ...
          if (!stock) {
            console.warn(
              `⚠ No se encontró stock para idStock: ${productoEliminado.idStock}`
            );
            continue;
          }
          ...
          if (pedidoEnStock) {
            ...
            if (pedidoEnStock.estado === "reservado") {
              ...
              console.log(
                "🔄 Re-evaluando pedidos pendientes después de liberar stock reservado..."
              );
              await evaluarPedidosPendientes(
                productoEliminado.idStock.toString()
              );
            ...
            console.log(
              "⚠ Pedido no encontrado en el stock, no hay nada que eliminar"
            );
          }
        }
      }
      ...
      if (productosNuevosAgregados.length > 0) {
        ...
        for (const productoNuevo of productosNuevosAgregados) {
          ...
          if (!modelo || !modelo.placas_por_metro) {
            console.warn(
              `⚠ No se encontró modelo para idModelo: ${productoNuevo.idModelo}`
            );
            continue;
          }

          const cantidadRealNecesaria =
            productoNuevo.cantidad * modelo.placas_por_metro;
          ...
          if (!stock) {
            console.warn(
              `⚠ No se encontró stock para idStock: ${productoNuevo.idStock}`
            );
            continue;
          }
          ...
          if (stockDisponible >= cantidadRealNecesaria) {
            ...
            console.log(
              `🟢 Producto nuevo RESERVADO para ${productoNuevo.idStock}: ${cantidadRealNecesaria} unidades`
            );
          } else {
      ...
      for (const productoNuevo of productosNuevos) {
        const productoAnterior = productosAnterioresMap.get(
          productoNuevo.idStock.toString()
        );

        if (
          productoAnterior &&
          productoNuevo.cantidad !== productoAnterior.cantidad
        ) {
          ...
          if (pedidoEnStock) {
            ...
            if (pedidoEnStock.estado === "reservado") {
              ...
              await Stock.findByIdAndUpdate(
                ...
                {
                  arrayFilters: [
                    { "elem.idPedido": pedidoExistente._id as any },
                  ],
                  new: true,
                }
              );
          ...
          console.log(
            "🔄 Re-evaluando pedidos pendientes para stock:",
            productoNuevo.idStock
          );
          ...
          if (pedidoEnStockActualizado) {
            const estadoFinal =
              pedidoEnStockActualizado.estado === "reservado"
                ? "Disponible"
                : "Pendiente";
            console.log(
              `🔄 Actualizando estado final del pedido a: ${estadoFinal}`
            );
          ...
          updates.productos.map(async (prod: any, index: number) => {
            ...
            if (esProductoNuevo) {
              ...
              const pedidoEnStock = stock?.pedidos.find(
                (p: any) =>
                  p.idPedido.toString() ===
                  (pedidoExistente._id as any).toString()
              );
              estadoCorrecto =
                pedidoEnStock?.estado === "reservado"
                  ? "Disponible"
                  : "Pendiente";
              console.log(
                `🔄 Producto NUEVO ${prod.idStock} - Estado del stock: ${estadoCorrecto}`
              );
            } else {
              // ✅ Es un producto existente - usar estado del pedido actualizado
              estadoCorrecto = productoActualizado?.estado_stock;
              console.log(
                `🔄 Producto EXISTENTE ${prod.idStock} - Estado correcto: ${estadoCorrecto}`
              );
            }
    ...
    console.log("🔄 === ACTUALIZANDO PEDIDO FINAL ===");
    console.log(
      "📦 Datos finales de actualización:",
      JSON.stringify(updatesFinal, null, 2)
    );
...
export const deletePedido = async (
  ...
  try {
    ...
    for (const producto of pedidoAEliminar.productos) {
      ...
      if (!modelo || !modelo.placas_por_metro) {
        console.warn(
          `⚠ No se encontró modelo para idModelo: ${producto.idModelo}`
        );
        continue;
      }
      ...
      if (!stock) {
        console.warn(
          `⚠ No se encontró stock para idStock: ${producto.idStock}`
        );
        continue;
      }
      ...
      if (pedidoEnStock) {
        ...
        if (pedidoEnStock.estado === "reservado") {
          ...
          console.log(
            `✅ Stock reservado liberado para ${producto.idStock}: -${pedidoEnStock.cantidad} unidades`
          );
          console.log(`✅ Pedido eliminado del array pedidos`);
          ...
          console.log(
            `✅ Stock pendiente liberado para ${producto.idStock}: -${pedidoEnStock.cantidad} unidades`
          );
          console.log(`✅ Pedido eliminado del array pedidos`);
        }
      } else {
        console.log(
          "⚠ Pedido no encontrado en el stock, no hay stock que liberar"
        );
      }
    }
    ...
    console.log(
      `✅ Pedido ${pedidoEliminado?.remito} eliminado correctamente.`
    );
...
export const añadirComentario = async (
  ...
  try {
    ...
    if (tipo === "cliente") {
      ...
      console.log(
        `✅ Comentario de cliente ${comentarioAnterior ? "reemplazado" : "añadido"
        } correctamente`
      );
      ...
      if (
        indiceProducto === undefined ||
        indiceProducto < 0 ||
        indiceProducto >= pedido.productos.length
      ) {
        res.status(400).json({
      ...
      const comentarioAnterior =
        pedido.productos[indiceProducto]?.comentario_producto;
      pedidoActualizado = await Pedido.findByIdAndUpdate(
        id,
...
export const eliminarComentario = async (
  ...
  try {
    ...
    if (tipo === "cliente") {
      ...
      console.log(
        `✅ Comentario de producto eliminado correctamente del índice ${indiceProducto}`
      );
    }
